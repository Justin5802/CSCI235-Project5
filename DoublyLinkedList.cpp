...
//  List
//  A doubly-linked list implementation for class project CSci 235 Software Design and Analysis II
//
...
void DoublyLinkedList<T>::swap(int i, int j)
{
    ...
    Node<T>* node1 = getPointerTo(i);
    Node<T>* node2 = getPointerTo(j);
    if (!node1 || !node2) {
        return;
    }
...
//helper function that sets index at position based on current status of linked list
template <class T>
void DoublyLinkedList<T>::setItem(int index, T value) {
    if (index >= 0 && index < getLength()) {
        Node<T>* curr = getHeadNode();
